...
- `supabase/migrations/0027_ea_subscription_forever.sql` — **RAN 2026-04-30, verified clean.** Snapshotted 33 EA rows into `_backup_ea_subs_2026_04_30`, nulled `subscription_expires_at` on all 33. Post-flight verification: `ea_with_lingering_expiry = 0`, `total_ea = 33` (matches backup), `paid_with_expiry = 0` (no non-EA paid subscribers yet — expected this early in the campaign).
...
- Telegram bot lapse-reminder code does NOT exist yet on lastproof side. Help-page copy describes future behavior (3-day reminder for paid subscribers, never for EA). Per the operator: defer — 1-2 weeks runway before 5K cap hits and the reminder becomes relevant.
...
- Cookie-based attribution had cross-browser / mobile-wallet-redirect /
  ad-blocker / private-mode failure modes the operator kept hitting.
...
— the operator runs in Supabase SQL Editor. Additive (nullable column +
partial index), safe against live data.
**Impacts:** none — work_items column add, no Terminal contract change
**Status:** ✅ shipped, awaiting the operator's migration run + live test
...
- **Phase 2-3 (solution)** chose dual-input flow with the operator:
  no Jupiter / no auto-resolve, operator self-attests CA when their
  ticker isn't in TOKEN_MINTS. Trust-safe (no impostor risk),
  ⅓ the code of auto-resolve, no external dep, matches the operator's
...
**Role:** backend
**Commits:** this commit (see git log)
**Migrations run in prod Supabase:** **REQUIRED** — the operator must run
...
- **Solscan link rendering** uses `formatSolscanLink()` which
  ...
  validation upstream; per the operator's spec, it's pure pass-through.
...
Per the operator, mid-session: *"we quietly go launch now... no one will even
...
- **`/grid` flipped from locked placeholder to entry door.** Boot
  ...
  ghost button stays. **No cookie gate** — the operator's reflection: "if
...
- **VERSION held at 0.13.2; `data/updates.json` untouched.** Per
  the operator: skip /status entry for quiet launch. The 1.0.0 bump + "Grid
  is live" updates entry will land together as one commit when he's
  ready to announce.

- **`IP_HASH_SALT` set on Vercel pre-launch** (the operator, self-serve in
...
- **Eventually announce:** when the operator wants public attention, single
...
- **Stage 3 (cookie gate boot ritual):** still future. Quiet launch
  works without it; if/when the operator wants direct /operators visits to
...
- **VERSION + `data/updates.json` are intentionally lagging behind
  ...
  that didn't follow the [update: …] convention because the operator chose
  to defer the announcement. Don't "fix" this by retroactively writing
  a /status entry — it will land as part of the operator's announce commit
  when he's ready. If you ship MORE user-visible behavior before that
  announce commit, decide with the operator whether to follow convention
...
- **`GRID_LAUNCH_DATE` constant** referenced in CLAUDE.md may still be
  ...
  pass when the operator does the announce commit. Not urgent — quiet launch
...
**Commits:** `5e6b6f8`
**Migrations run in prod Supabase:** `0025_normalize_profile_timezone.sql` — **NOT auto-run; the operator applies manually before smoke test**
**Impacts:** none on Terminal
...
doesn't have, etc). the operator flagged: "are you quick fixing or putting
...
- **SHIFTBOT-ranked mode lock.** When `?ranked=h1,h2,…` is in URL
  ...
  only ranked mode locks. Per the operator: "[Flag 2 = (a)] disable but
...
- **Migration `0025_normalize_profile_timezone.sql`** — strips
  ...
  statements in a transaction. **the operator applies manually before
  smoke test.**

- **Card meta row split.** Per the operator: full language names on cards
...
- **Dashboard editor (IdentityCard)** timezone dropdown gains the
  same city-annotation labels as Onboarding (the operator's call: "yes just
  for the dashboard"). Stored value remains bare offset.

### Open / next

- **the operator applies migration `0025` in Supabase**, then runs smoke
...
**Migrations run in prod Supabase:** `0024_shiftbot_refusals.sql` (applied earlier this session per the operator confirmation)
...
- **No `data/updates.json` entry, no VERSION bump.** Per the operator pre-launch
...
- **Added `validateDisplayName()`** to `src/lib/handle-validation.ts`,
  ...
  2. **Block invisible / RTL-override / control characters** —
     ...
     `the operator` vs. `kel‎len` with an invisible joiner).
...
- **Added `RESERVED_PERSONAL_NAMES` list** in the same module —
  ...
  display names. Currently seeded with `the operator` (the operator's
  surname; uncommon enough that false-positive risk is low,
  identity-distinctive enough that it cleanly catches
  impersonation attempts). First name `the operator` deliberately NOT
...
- Personal-name protection is bidirectional: a user can't pick
  `the operator` as either their handle OR their display name, can't
...
- Existing 32 profiles audited yesterday — the operator's own profile
...
- **Audit existing display names** — the operator said skip but worth
...
- **First-name founder protection** — currently I only added
  `the operator` to `RESERVED_PERSONAL_NAMES`. If `the operator` alone
  becomes a problem (e.g. someone picks `the operator_lastshift`-style
  but with brand term swapped to evade), add it too. Tradeoff
  is real the operators get blocked. Defer until evidence of need.
...
- **`the operator` is the only personal name in the list right now.**
...
- **New module: `src/lib/handle-validation.ts`** — single source of
  ...
  2. **Reserved brand terms** — substring match against the
     ...
     surrounding context. Reserved list (per the operator's spec):
...
- **User-facing rejection message is generic and identical for all
  ...
  Per the operator's spec — don't tell attackers which rule blocked them
...
- Existing handle audit pending — the operator will run
...
- **Slur list reviews** — eight terms is a starting point. If real
  users report false positives or the operator wants to tighten further
...
- **`@lastshiftfounder` — the operator's own handle — would itself be
  rejected by the new founder-spoof rule** (distance 0). Doesn't
  matter in practice because the operator already owns it; he won't
  ...
  Per the operator's spec, no admin override path was added — system
...
Frontend-lane work. Crossed the lane with the operator's explicit OK because:
...
**Status:** ✅ pushed; awaiting the operator to run the one-time backfill SQL
...
Locked decisions per the operator this session:
- Default chad is hardcoded to `lastshiftfounder` (no env var)
- Re-adds go through the normal ask flow — the operator accepts
...
### Backfill SQL (one-time, the operator runs in Supabase dashboard)
...
- VERSION still at `0.12.2` — the operator's call whether to add a new
...
**Status:** ✅ pushed; awaiting the operator call on updates-feed entry + flag flip

### Did

Rebuilt chad-function semantics from "mutual" → "directional one-way"
(Instagram-private style) per the operator's locked instruction:
...
Plus three vocabulary changes the operator specified:
...
- `src/components/chad/ChadManagementStrip.tsx` — title
  ...
  variants. Both counts kept (per the operator option a).
...
- `src/components/chad/ChadEmptyState.tsx` — dashboard context
  copy now matches the operator's spec; public context copy adjusted to
...
- Schema (`0021_chads.sql`) — unchanged. The directional model
  ...
  to change.
- `VERSION` — the operator's call whether the relaunch needs a new
...
- Modal copy — `pending` phase still says "Request already sent"
  and "Request Pending". the operator specified vocabulary changes for
...
- **the operator call: updates feed entry?** The 0.12.0 entry on
...
- **Redeploy + flip CHADS_ENABLED=true** when the operator is ready.
...
- **Dashboard strip title is now `PENDING ASKS`** but the inline
  ...
  This is intentional per the operator's "option a" (keep both counts).
...
the operator launched chad in prod (CHADS_ENABLED=true), did a real smoke
...
- the operator's actual intent is **directional independent** — A→B and
  B→A are separate, independently-accepted requests. One accept
  does NOT auto-list both sides.

the operator's quote: *"i request him and he request me should never be
...
- One real chad row in prod Supabase: `(lastshiftfounder,
  bossvito, pending)` — the operator's test request. Bossvito hasn't
...
1. **Lock the directional-independent model.** Confirm Option 1
   semantics with the operator before any code touches:
...
This is the third design miss I owe the operator on this feature. First:
...
neither I nor Cowork explicitly verified with the operator what "Chad
Army" means semantically. Tomorrow's first action: confirm the
locked model in writing with the operator, update the brief, then build.
...
("any questions for backend?") and treated the operator's "rest can go to
...
2. the operator sets `CHADS_ENABLED=true` in Vercel env (Production scope)
...
If the operator flips the env var BEFORE this commit's deploy lands, no
...
pending tile borders, and modal phase 9. Per the operator, this is a
...
Walked the operator through wireframes #1–#6 of the Chad Function build one-by-one
...
- Gold-as-attention extension (pending border, pending count, pending handle)
  ...
  the operator's call is "leave it as is" for now across all 6 wireframes.
