...
## 2026-04-24 10:16 MST — Ambassador attribution: /manage reflection completes the URL chain
...
**Impacts:** none — proxy-only extension, no contract/schema change
**Status:** ✅ shipped

### Did

- **Gap from 10:05 MST commit (`c749b4d`):** that one handled URL
  reflection on ambassador surfaces (campaign slugs + profile
  handles) but NOT on /manage. So the flow was:
  1. User on /@habilamar_ibn → 307 → /@habilamar_ibn?ref=free-before-grid ✓
  2. User clicks CONNECT WALLET → navigates to /manage
  3. Address bar shows /manage (no ref) ✗
  4. User gets stuck on wallet connect in in-app browser
  5. Copies /manage from address bar, pastes in Chrome → cold
  the operator flagged it: the URL chain breaks at the moment the user
  most likely copies (when stuck on /manage).
- **Fix:** extended `src/proxy.ts` matcher to include `/manage`,
  and added a new pre-existing-surfaces branch in the proxy:
  - If path is `/manage` AND URL has no `?ref=` AND `lp_ref`
    cookie has a value → 307-redirect to `/manage?ref=<cookie-value>`.
  - If URL already has `?ref=` → skip (idempotent).
  - If no `lp_ref` cookie → skip (cold /manage visitor, nothing to
    reflect).
- **Attribution chain now end-to-end in the address bar:**
  ```
  /@habilamar_ibn
    ↓ (proxy redirects, URL bar shows)
  /@habilamar_ibn?ref=free-before-grid
    ↓ (user clicks CONNECT WALLET)
  /manage
    ↓ (proxy redirects based on cookie, URL bar shows)
  /manage?ref=free-before-grid
    ↓ (user gets stuck, copies URL, pastes in Chrome)
  /manage?ref=free-before-grid IN CHROME
    ↓ (register_tid reads body.ref)
  Attribution stamps ✓
  ```
- **Observability:** new `proxy_touch` events for /manage
  reflections carry `metadata.surface = "manage_reflection"` so
  they're distinguishable from campaign/profile first-touches in
  the funnel query.
- **Edge cases verified:**
  - Authenticated user on /manage — their `lp_ref` cookie was
    already consumed by register_tid at first auth (per the 2026-04-22
    "cookie consumed on register_tid" logic). No cookie → no
    redirect. Clean.
  - Non-referred /manage visitor — no cookie, no redirect,
    /manage stays as-is. Zero impact on cold traffic.
  - Returning user with expired session but lingering cookie —
    sees reflection in URL, re-stamp on next register_tid. Desired.
- **Updates feed convention applied:**
  - `VERSION` 0.11.6 → 0.11.7 (patch, category=improved)
  - `data/updates.json` entry at top, `latest_version` bumped
  - `[update: improved]` prefix on the commit subject
  - Copy mentions only the user outcome (ambassador still gets
    credit after browser switch), no mechanism leak.

### Current state

- The URL chain for ambassador-referred users is now continuous
  from the first tap through every server-side surface they reach:
  ambassador page → /manage → all show `?ref=<slug>` in the
  address bar.
- Cross-browser copy-paste case fully covered: user stuck on
  /manage in-app, copies address bar, pastes in Chrome → Chrome
  loads /manage with ref → register_tid stamps attribution.
- No UA detection anywhere in the chain. Pure cookie → redirect
  mechanics.

### Open / next

- **Audit after 24h of prod data:**
  ```
  ...
  GROUP BY surface, campaign_slug
  ORDER BY n DESC;
  ```
  Expected shape: campaign_page hits (first touches from campaign
  URLs), profile_page hits (first touches from /@handle),
  manage_reflection hits (cookie propagation). Ratios tell us how
  the three surfaces are carrying traffic.
- **Other reflect-worthy pages.** If users get stuck on any other
  page while waiting-to-authenticate, the same reflection pattern
  applies. Candidates: `/manage/profile`, `/manage/safety`. Not
  shipping today; they're less likely to be the "copy from address
  bar" moment. Flag if observability data shows these matter.
- **Home page (`/`) reflection** — intentionally NOT adding.
  Reflecting `?ref=` on the home page for any cookie holder would
  add attribution noise to the single most-linked URL on the site.
  Home page is the "anyone arrives here cold" surface; attribution
  routes through explicit ambassador surfaces only.

### Gotchas for next session

- **Two redirect hops happen on the ambassador-referred
  happy path**: one on the ambassador surface (adds ?ref to the
  URL), one on /manage (re-adds ?ref= from cookie). Both are 307,
  both preserve method, both are invisible to users beyond a
  potential URL flicker. If performance data flags the double
  redirect, consider having the ambassador surface set a hint
  that /manage respects to skip its redirect on the same session.
- **The `/manage` matcher does NOT make /manage appear in the
  proxy_touch "campaign visited" metrics**. Its events have
  `surface = "manage_reflection"`, source `"cookie"`, outcome
  `"already_stamped"`. Queries that count "ambassador landings"
  should filter `surface IN ('campaign_page', 'profile_page')`
  explicitly, excluding manage_reflection.
- **If the /manage page ever adds functionality that reads
  `?ref=` from the URL directly**, make sure it reads from
  `searchParams.get("ref")` and doesn't assume the param is
  absent. Our existing /manage client already does this
  correctly via the `ref_slug` prop, no change needed there.

---

## 2026-04-24 10:05 MST — Ambassador attribution: URL reflection for cross-browser survival
...
**Commits:** this commit (see git log)
**Migrations run in prod Supabase:** none
**Impacts:** none — proxy-layer + metadata change, no contract/schema change
**Status:** ✅ shipped

### Did

- **Problem framing** (credit: the operator iterated me toward this):
  attribution was losing the cookie-jar-crossing case even after the
  profile-page proxy fix. The precise failure:
  1. User taps `/@habilamar_ibn` in a Telegram/X/Discord in-app browser
  2. Cookie set in the in-app cookie jar
  3. User tries to connect wallet → blocked (in-app browsers are
     read-only for wallet extensions)
  4. User sees a "stuck?" prompt OR figures it out themselves
  5. User copies the URL **from the browser's address bar**, not from
     any in-app UI button
  6. Pastes in Chrome → URL is whatever the address bar showed,
     which was just `lastproof.app/manage` or `/@handle` — no ref
  7. Chrome loads cold → register_tid sees no attribution

  And the underlying structural limitation: **UA-based in-app browser
  detection is unreliable enough that we should treat it as 0%
  accurate for design purposes.** Fixes that depend on detecting
  in-app (banner copy tweaks, bespoke UI, etc.) fail silently for
  users whose UA we don't recognize — which is the majority on iOS
  now that WKWebView-based apps report Safari-identical UAs.

- **Fix:** **URL reflection via 307 redirect in the proxy.** When a
  user lands on any of the 12 ambassador-surface paths (6 campaign
  slugs + 6 profile `/@handle` routes) and the URL does NOT already
  contain `?ref=`, the proxy 307-redirects to the same path with
  `?ref=<slug>` appended.

  The address bar then shows the attributed URL for the entire
  session. Any subsequent copy of the URL — from the address bar,
  via "Share" menu, by forwarding to someone else — carries the ref.
  When pasted into Chrome, register_tid reads `body.ref` and stamps
  attribution exactly as if the user had arrived via a CTA-appended
  URL.

  Critically: **no UA detection, no banner dependency, no client
  JavaScript required.** Runs server-side on every matched-path
  request. Idempotent: the second pass (URL now has `?ref=`) skips
  the redirect and proceeds to existing cookie-set + observability
  logic.

- **SEO preservation:** added `alternates.canonical` to the profile
  page metadata pointing at the clean `/@handle` URL. Prevents
  Googlebot from indexing `/@handle?ref=<slug>` as a duplicate of
  the clean URL. Users see the ref; search engines see the canonical.

- **Observability extended:** `proxy_touch` events now log
  `url_ref` and `url_ref_matches_slug` in metadata. Lets us
  distinguish:
  - User arrived with matching URL ref (happy path)
  - User arrived with mismatched URL ref (typo or pasted-wrong)
  - User arrived with no ref (pre-redirect — should be rare, only
    first visit)

- **Updates feed convention applied:**
  - VERSION 0.11.5 → 0.11.6 (patch, category=improved)
  - `data/updates.json` entry at top, `latest_version` bumped
  - `[update: improved]` prefix on the commit subject
  - Copy framed from user's perspective — no mention of
    "redirect" or "cookie", just "referral survives browser switch"

### Current state

- Every ambassador surface visit results in an address bar showing
  `?ref=<slug>` — campaign pages AND profile pages alike.
- Cookie + URL both carry the ref from that point. Redundancy is
  intentional: cookie works for within-browser navigation; URL
  works for cross-browser copy/paste.
- Canonical URL pinned to clean form for SEO.

### Open / next

- **Run the first query in 24 hours:**
  ```
  SELECT event_type, campaign_slug, metadata->>'url_ref_matches_slug' AS matched,
         count(*) AS n
  FROM referral_events
  WHERE event_type = 'proxy_touch'
    AND created_at > now() - INTERVAL '24 hours'
  GROUP BY event_type, campaign_slug, matched
  ORDER BY n DESC;
  ```
  Tells us how many proxy_touch events are catching a user who
  arrived via an already-attributed URL (good — means the redirect
  propagated) vs needing the redirect fresh.
- **Consider adding the same redirect treatment to `/manage`**
  when the `lp_ref` cookie is present. Catches the case where a
  user somehow bypasses ambassador surfaces but has the cookie.
  Low priority; not shipping today.
- **Home-page pre-ambassador-attribution link sharing** — if a user
  shares `lastproof.app/@habilamar_ibn?ref=free-before-grid` with a
  friend, the friend also arrives with the ref pre-set. First-touch
  wins still applies at the operator level — friend gets attributed
  to habilamar regardless of who sent them the link. Intentional;
  the "who introduced this user to LASTPROOF via any path" question
  answers via the ambassador-surface cookie.

### Gotchas for next session

- **The redirect is the first thing users see on any ambassador
  surface** — a brief 307 hop before the page renders. This is
  normal and invisible to users beyond a potential momentary URL
  flicker. If anyone asks "why do I see `?ref=` in my URL," the
  honest answer is "so we can credit the right ambassador even if
  you share this page somewhere weird." It's not tracking in the
  surveillance sense.
- **Canonical URL is critical for SEO.** If you ever add a new
  ambassador surface (say, `/5k/<slug>`-indexable variant), add
  the same `alternates.canonical` to the metadata pointing at the
  clean URL. Without it, Google may index the `?ref=` version.
- **Redirects in Next.js middleware/proxy affect client-side
  navigation too.** `<Link>` prefetch and route transitions will
  follow the 307. Tested path: click a Link to an ambassador
  surface → prefetch may hit the redirect → client navigation
  lands on the `?ref=` URL. This is the desired behavior; flag if
  it misbehaves in practice.
- **Don't 307 when `?ref=` is present even if the value looks
  wrong.** We respect any existing `?ref=` regardless of whether
  it matches the path's slug. `register_tid` validates the slug
  against the `ambassadors` table server-side and rejects invalid
  ones; the proxy doesn't need to second-guess. Intentional: URL
  param is the explicit user-visible attribution signal; the
  proxy shouldn't silently overwrite it.
...
**Role:** backend
**Commits:** this commit (see git log)
**Migrations run in prod Supabase:** one manual backfill by the operator in
...
During the grid session, the operator corrected a source-of-truth mental model
...
the operator: *"wireframes are never never never true representation of what is
...
`/Users/the operator2023/.claude/projects/-Volumes-LASTSHIFT/memory/feedback_wireframes_not_production.md`
...
the operator across the session — the shipped state below reflects the
...
### Design decisions called out by the operator mid-session
...
- **REMOVE is instant, no undo.** The earlier wireframe had a 5-second
  countdown ring with UNDO. the operator killed it — instant hard-delete on
...
- **Modal phase 9 (no profile) leads with profile creation.** The
  earlier copy leaned "chad up" — the operator wanted the pitch inverted so
...
- Grid planning artifacts (`docs/GRID-*.md`, `wireframes/_drafts/`,
  ...
  NOT touch them — confirmed with the operator up front. They got committed
  by the operator's grid session (`d7d2ebc`) during my session.
...
**Migrations run in prod Supabase:** none
**Impacts:** none — internal docs + new `_drafts/` folder convention; no shared-contract changes, no Terminal touches, no user-visible surface changes (Updates feed convention explicitly skipped per the operator)
**Status:** ✅ planning block complete, standing by for Frontend verification pass on the wireframe before Phase 2 code kicks off

### Did

Ran the full Grid planning block end-to-end. Session pattern was: diligence → lock 20+ product decisions → write Cowork brief → Cowork drafts wireframe → the operator overrides → write Frontend verification brief. No code touched.
...
- No memory files existed at
  `/Users/the operator2023/.claude/projects/-Volumes-LASTSHIFT/memory/` — system
...
- Frontend handoff assumed Grid would live inside `(marketing)` inheriting
  ...
  go inside `(marketing)` (the operator's call — marketing-aesthetic, not terminal);
...
- Sort: dropdown (per the operator override) with 4 options — `Relevant` / `Most Trusted`
...
- Filter sidebar: 7 sections (Tier / Verified / DEV Proofs / # of Proofs / Fee /
  ...
  ~240px wide. the operator reordered so Verified is at position #2 for discoverability
- DEV Proofs filter: binary toggle (per the operator override), not a range slider
...
- Grid Status widget: DROPPED (the operator's call — cleaner sidebar)
...
(the operator-facing intro + `═══ BRIEF CONTENT FOR COWORK ═══` divider → spec body).
...
the Cowork brief that the operator explicitly confirmed as intentional overrides:
...
These three are LOCKED per the operator. Any future session that tries to "restore"
...
parity, filter interactions, ticker + chips). the operator's 3 overrides called out
...
disk — the operator copied it directly when dispatching). Explicit rules: read file
...
- Working tree carries the new files as untracked adds — the operator's decision
...
- the operator hands `docs/GRID-TO-FRONTEND-HANDOFF.md` to a fresh Frontend session.
  Frontend returns polished wireframe at
  `wireframes/_drafts/grid/lastproof-grid-fe-verified.html`.
- the operator relays back to grid session for final canon adherence check.
...
- **The three the operator overrides are locked** — sort dropdown (not pills), DEV
  ...
  they're reverting the operator's explicit call. Both the brief AND the Frontend
...
- **"New Profiles" is explicitly not a sort option.** the operator's call: new
...
- Two unrelated auto-named projects remain in the deployment
  ...
  the operator will verify and clean up manually.
...
**Device:** the operator's MacBook Air (`the operators-Air.lan`, macOS 26.4.1)
...
**Role:** backend
**Commits:** this commit (see git log)
**Migrations run in prod Supabase:** one manual stamp by the operator in Supabase
...
- **Diagnosed a real attribution gap.** the operator reported `@namesake01`
...
- **`wallet-gate` preserves the cookie** if the wallet has no operator
  ...
  when the row lands.
- **the operator ran the namesake01 backfill SQL** in Supabase SQL Editor.
...
- **Working tree hygiene handled on arrival.** Local files were
  ...
  session's explicit "the operator's call" note.
...
## 2026-04-22 01:18 MST — /help: place 23 new screenshots from the operator, wire last 5 Shot calls
...
- the operator dropped 23 final screenshots into `wireframes/help images/`
...
- Source PNGs in `wireframes/help images/` can be deleted or kept
  ...
  Leaving them for now; the operator's call whether to clean up.
- Terminal shots (T1 Steps 01–03) previously flagged as blocked on
  Terminal builder — now resolved with the operator-provided captures.
- No remaining placeholder Shots in the /help render tree.

### Gotchas for next session

- **`/public/help/` filenames are the contract** between `page.tsx`
  Shot calls and the asset files. If the operator swaps in new versions,
...
- the operator flagged that /help was missing the centered reading column
...
**Device:** the operator's Mac mini
...
- No migrations, no Terminal contract changes, no touching other
  pages or routes.
- Route `/help` renders standalone; ready for the operator's review via
  `pnpm dev` or Vercel preview.

### Open / next

- the operator reviews rendered /help. Iterate on any copy / layout tweaks.
...
- the operator: the SHIFTBOT pinned strip was missing from /help even
...
- Prior entry (02:26 MST) pulled the footer from
  ...
  `globals.css .lp-footer` have drifted since. the operator flagged:
...
**Device:** the operator's Mac mini (`the operators-Mac-mini.local`)
**Platform:** Claude Desktop
**Model:** claude-opus-4-6
**Role:** help-page
**Commits:** this entry
...
**Device:** the operator's Mac mini (`the operators-Mac-mini.local`, macOS 15.3.1, account `the operator2023`)
**Platform:** Claude Desktop
**Model:** claude-opus-4-6
**Role:** help-page
**Commits:** this entry
...
- the operator flagged: "the entire lastproof platform is all coded images
...
- Replaced 24 placeholders:
  ...
  - **FAQ (1)**: "NO REGISTRY MATCH" error log matching the operator's
...
**Device:** the operator's MacBook Air (`the operators-MacBook-Air.local`, macOS 26.4.1)
**Platform:** Claude Desktop (`CLAUDE_CODE_ENTRYPOINT=claude-desktop`)
...
- the operator caught me using `SHIFT-XXXX-XXXX-XXXX-XXXX` as the Terminal ID
...
- Relayed a cross-session message through the operator to the Terminal builder.
...
- Per the operator's explicit narrow-scope instruction ("fix the place you got
...
explicitly left it alone per the operator's scope narrow:
...
- `wireframes/help-CONTENT.md`, `wireframes/manage-wallet-auth-flow.html`,
  ...
  per the operator's narrow-scope instruction. Future session can sweep these
  if desired.
...
- **Replaced `/help` topbar with the homepage global header verbatim**
  so there's one consistent site-wide header. the operator was specific: the
...
- Rewrote `wireframes/help.html` per the operator's feedback on the first
...
- **Topic 3 sub-topic verification** — some sub-topic affordances
  ...
  dashboard sections that the operator confirmed exist. Frontend builder
...
- Resolved all 10 open questions in the content doc via direct answers from
  ...
  10. Research from the prior thorough-research agent was **stale on three
      ...
      - `payer_wallet` column exists on `proofs` table — migration
        ...
        closed. Wrote a short explanation in the doc so the operator has the
...
**Device:** the operator's MacBook Air (`the operators-MacBook-Air.local`, macOS 26.4.1 arm64)
**Platform:** Claude Desktop (`CLAUDE_CODE_ENTRYPOINT=claude-desktop`)
**Model:** claude-opus-4-6
...
- Onboarded the three active LASTPROOF sessions on the mac mini
  (`the operators-Mac-mini.local`, macOS 15.3.1, account `the operator2023`) to
...
- Closed the `terminal-build` stale-worktree mystery: the
  `/Users/the operator2023/…/worktrees/sharp-hypatia` ghost reference
  tracked back to the same mac mini (macOS account name is
  `the operator2023`, not a different user). the operator confirmed: one person,
  two machines.
...
- **Cross-machine commit author drift** — commits from the MacBook
  Air come out as `the operator <the operator@…>`, commits from the mac mini
  will come out as `the operator <the operator@…>` (or `the operator2023`), same
  person. Not fixed yet. Consider running
  `git config --global user.name "the operator"` + `user.email "..."` on
