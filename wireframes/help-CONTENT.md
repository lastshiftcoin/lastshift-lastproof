...
3. Add a short comment (optional, up to 140 chars) — e.g. "@the operator ran our Tier 1 raid team for 6 months. Real one." This shows up on the proof card publicly.
...
**Visual (inline):** a cropped thumbnail of the exact /manage error state (the one from the operator's 2026-04-19 screenshot — the orange "New wallet detected [NO REGISTRY MATCH]" log lines). Makes it obvious this FAQ matches what the user is looking at.
...
### Right — Email support
**Label:** `EMAIL`
**Address:** `help@lastshift.ai` *(or TBD — confirm with the operator)*
...
| V8 | Step 04 — Register on LASTPROOF | Split: `/wireframes/lastproof-onboarding.html` + the operator's /manage screenshot (2026-04-19) | Before: paste-Terminal-ID state. After: fresh dashboard. | Two browser chrome frames side-by-side with "→" between |
...
| V11 | FAQ — "Operator not found" error | the operator's screenshot (2026-04-19, /manage error state) | The exact orange error log section cropped out | 240px wide |
...
All 10 original open questions answered by the operator:
...
4. ✅ Add a "Last updated" timestamp at the page footer — matches the pattern the operator thinks exists on the dashboard. If no existing pattern is found when the frontend builder implements, fall back to a static ISO date rendered server-side from git commit date.
...
### Explanation for the operator — what `payer_wallet` was about
