...
/**
 * /[campaignSlug] — Ambassador campaign landing page at root URLs.
 *
 * e.g. lastproof.app/early-access-free, lastproof.app/first-5000-free
 *
 * Validates the slug against the ambassadors table. If it's a known
 * campaign_slug, renders the same landing page content as /lastproof/[slug].
 * If not, returns 404 — so this doesn't catch real routes like /manage.
 *
 * noindex: prevents search engines from indexing campaign pages,
 * which would cause false attribution from organic traffic.
 */
...
export default async function CampaignLandingPage({ params }: PageProps) {
  ...
  // Validate against ambassadors table
  const { data: ambassador } = await supabaseService()
    .from("ambassadors")
    .select("campaign_slug, is_active")
    .eq("campaign_slug", campaignSlug)
    .eq("is_active", true)
    .maybeSingle();
