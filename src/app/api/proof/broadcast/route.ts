 ...
 *
 * Accepts a wallet-signed transaction (base64), submits it to the
 * Solana RPC via sendRawTransaction, and returns the signature.
 * The frontend then polls /api/proof/tx-status for confirmation.
 *
 * Response shape matches the mock at /api/mock/proof/broadcast:
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[broadcast] sendRawTransaction failed:", msg);
