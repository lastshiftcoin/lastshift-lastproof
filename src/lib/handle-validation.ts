 ...
 * specific rule that fired, no support contact path. Per the operator's
 ...
 *
 * Surname-only because first names like "the operator" are too common to
 * blanket-block. Surname "the operator" is uncommon enough that
 * false-positive risk is low. Anyone legitimately named the operator will
 ...
 * (e.g. `the operator` vs. `kel‎len` with an invisible joiner).
 */
