...
ARE the spec. the operator is the source of truth for both. Your job as
...
1. **Modal connect screen redesigned without approval** — the
   ...
   fork. Rolled back in a separate commit when the operator called it out.
...
2. **Notification fanout invented from thin air** — `chad_request`
   ...
   wireframe or brief. The builder treated the operator's "rest can go
   ...
   when the operator called it out.

3. **Wrong relationship model shipped** — the chad data model was
   built as "mutual" (one row per pair, one accept lights both
   armies). the operator's actual intent was "directional independent"
   (each side requests + accepts separately). Both Cowork's
   wireframe and the builder's read of the brief assumed mutual
   without verifying the semantic with the operator. Surfaced when the
   feature went live and the operator smoke-tested it. Required a model
   rebuild.

Each was preventable by a single explicit question to the operator before
