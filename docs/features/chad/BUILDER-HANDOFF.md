...
- **Remove = instant, no undo, directional only.** the operator's explicit
...
- No count, no "12" — per the operator's direction.
...
- **Avatar ordering:** intentionally random in production (per the operator).
...
### Key decisions (the operator called-outs)
...
// response from GET /api/chads/eligibility?target=<handle>
{
  ...
  "viewer": { "handle": "the operator", "wallet": "F7k2...9xMp" },
...
wireframe, removed per the operator. Now:
...
row via `/api/chads/remove`. the operator's explicit call. The earlier
...
2. **Inline undo on Remove was rejected** — current design is instant
   ...
   revisit — but per the operator this was deliberate.
