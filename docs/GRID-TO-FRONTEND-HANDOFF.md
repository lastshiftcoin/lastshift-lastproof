...
**Status:** Wireframe locked by the operator; Frontend verifies only — do NOT redesign

---

## How to use this brief (for the operator)
...
the operator made three intentional overrides on top of Cowork's draft. Frontend's first instinct will be to "restore" them to the Cowork brief's spec. **Don't.** These are locked:
...
```
the operator  →  pastes brief below to Frontend
...
the operator  →  relays to grid session for final canon check
Grid    →  approves → Phase 2 build begins
         → or loops back to the operator with revisions
```
...
You're the frontend specialist on LASTPROOF. A new wireframe for `/grid` (launching 2026-05-08) was produced by Cowork from a detailed brief, then the operator made three edits on top. Your job is a **brand / token / styling verification pass** — audit the wireframe against LASTPROOF's visual canon and either refine it in place or flag issues.
...
## Three overrides the operator locked on top of the Cowork brief
...
If you think one of these should be something else, flag it to the operator — don't silently change it.
...
- Don't touch the 3 the operator overrides (sort = dropdown, DEV = toggle, section order)
...
```html
<!-- FLAGGED FOR the operator: filter row touch targets are 36px on mobile — needs scale-up at ≤640px -->
```

Or return them as a bulleted list in your reply to the operator.

**When done, tell the operator:**
1. The file is saved at the path above
2. Short summary of what you changed
3. Any unresolved items flagged for the operator's decision

---

*Brief prepared by the grid session, 2026-04-23. Frontend verifies only — does not redesign. the operator approves the final before the grid session starts Phase 2 code.*
