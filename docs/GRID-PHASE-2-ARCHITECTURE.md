...
Per CLAUDE.md / the operator's framing: `/grid` is the door, `/operators` is the room. URLs describe what each surface does. The "Grid" brand concept lives in marketing copy, the boot screen, the homepage `SCAN GRID` button — never needs URL real estate.
...
No `New Profiles` sort — the operator's call: visibility is earned through proofs, not granted to new arrivals.

---

## 7. The 7 filter sections (LOCKED)

Order in sidebar matches the wireframe (Verified at #2 per the operator override):
...
| 3 | DEV Proofs | toggle (binary, per the operator override) | `dev_proofs_confirmed > 0` derived |
...
Counts next to filter options ARE in the wireframe (e.g. `TIER 4 · LEGEND (31)`) but the operator later removed them from the design. There is exactly ONE count surface on the page — the aggregate `<b>297</b> Profiles` next to the sort dropdown — Shopify-style. That count updates as filters apply, computed client-side from `filtered.length`.
...
Per the operator's call: SEO is handled by blog + homepage. The Grid doesn't need to be indexed. No Googlebot bypass in Phase 2. If/when discovery indexing matters, add a UA check in the proxy.
...
Per the operator's call: cron-cached counts, not on-demand aggregation.
...
Per the operator's iterative deploy plan from the 2026-04-24 session:
...
- No internal links to `/operators` yet — only the operator knows the URL

**Output:** the operator can hit `lastproof.app/operators` and see the polished Grid running on real domain/CDN/fonts.
...
- Update `src/app/sitemap.ts` to include `/operators` (or leave out — the operator's call)
...
- Sort = dropdown (the operator override)
- DEV Proofs = binary toggle (the operator override)
- Verified at sidebar position #2 (the operator override)
...
*End of architecture reference. Implementation kicks off with Stage 1 visual scaffold once the operator green-lights Phase 2 build.*
